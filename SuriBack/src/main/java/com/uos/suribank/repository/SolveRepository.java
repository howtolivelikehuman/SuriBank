...
public class SolveRepository extends QuerydslRepositorySupport{
    ...
    public solveDTO findAnswer(Long user_id, Long problem_id){
        ...
        return queryFactory.from(qUserPSolve)
                .select(Projections.constructor(solveDTO.class, qUserPSolve.id, qUserPSolve.problem.id,qUserPSolve.user.id,qUserPSolve.user.nickname,
                        qUserPSolve.userAnswer, qUserPSolve.score, qUserPSolve.solveDate))
                    .where(qUserPSolve.user.id.eq(user_id)
